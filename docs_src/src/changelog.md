...
- The blocked pieces framing the gate of a walled town siege are now `a gatepost` on the cursor and `gateposts, 2 cells, impassable` in the scanner, in every theme, instead of the bare `impassable` they used to read as. See [Terrain vocabulary](combat.md#terrain-vocabulary)
