...
- A mod gesture rebound to a punctuation key now also matches by the character the key prints, so a chord captured on a keyboard whose backslash is an OEM key works on one where it is the Backslash key, and the other way round. A rebound gesture no longer keeps the backslash fallback of its old default
