...
- A battlefield feature is now measured along its own axis, so a band of raised ground running diagonally is a `diagonal ridge` rather than a patch, and one running up the board a `vertical ridge`. In combat the cursor and the scanner name what blocks a hex - `boulders, impassable`, `torch, impassable`, `wall of 6 bushes and boulders, impassable` - in the words of the faction theme the battle inherited; the deployment grid still says `impassable`, since its preview draws every blocked hex the same way. `Ctrl+D` in combat now also says what lies around the board: `The battlefield is surrounded by forest to the west and north, mountains to the east.` See [Terrain vocabulary](combat.md#terrain-vocabulary)
- The battlefield's ground is now read outward from the spawn points, which is where a troop is set down: a layout raised from edge to edge used to come back as one cliff covering the whole board, its spawn points included. Walkable ground nothing can reach - a pocket sealed behind cliffs or impassable hexes - is now `unreachable ground` on the cursor, in the scanner and in the layout descriptions instead of passing for ordinary ground. It thuds under the tile cues the way a cliff does, since nothing can ever be standing there either. Two choke points sharing a hex are one crossing rather than two. In combat a moat or a line of fire is now described as `water` and `fire` rather than as a wall of them. See [Terrain vocabulary](combat.md#terrain-vocabulary)
- The cursor now says `choke point` on a hex everything has to pass through, on the combat grid and the deployment grid, where before only the scanner and the written descriptions knew about them. It comes after the ground the hex is, as in `elevated ground, height 1, choke point, 4, 4`, and it is a `Choke point` element in `Tile announcements` on the combat and troop deployment tabs, so it can be turned off, reordered or given a suffix like any other element. See [Terrain vocabulary](combat.md#terrain-vocabulary)
- An impassable hex no longer says how high it stands, on the combat grid or the deployment grid: nothing will ever be standing there. See [Terrain vocabulary](combat.md#terrain-vocabulary)
- The blocked pieces framing the gate of a walled town siege are now `gatepost` on the cursor and `gateposts, 2 cells, impassable` in the scanner, in every theme, instead of the bare `impassable` they used to read as. See [Terrain vocabulary](combat.md#terrain-vocabulary)
- What blocks a hex is now a bare noun where it is spoken as a label and a noun phrase where it is spoken as prose: the cursor and the scanner say `torch, impassable` and `knight statue, impassable`, while a written layout description still reads `a torch` and `a wall of bushes`. See [Terrain vocabulary](combat.md#terrain-vocabulary)
