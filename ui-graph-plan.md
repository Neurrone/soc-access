...
| `CommanderSheetScreen` | InventoryGrid, Menu, Buttons, Text | PROPOSED 2026-09-07 (revised: ES2's gesture family and usage hints adopted, no mod-authored drop targets), awaiting approval: five stops in drawn order: overview (tutorial button, Stats, Specialization, one modifier node stepping the game's `SwitchTab` axis with the active tab's rows under the drawn title), Equipment (one node per slot in drawn order, a two-hander's Main and Off Hand merged into one `InventorySlots/BothHands` node standing for the main hand), Inventory (one node per backpack position, the graph position replacing "slot N"), Skills then Powers (filled entries only), Close. Gestures are the game's own clicks: Enter = left click, Ctrl+Enter (`ui_alternate`, new) = Ctrl+left-click = drop on the ground, Backslash = right click = equip/unequip/use, Ctrl+Backslash (second binding of `ui_right_click`) = Ctrl+right-click = destroy (sell in a shop); the physical modifier is read by the game's own handler. Auto arrange (middle click) needs a chord decision. Occupied slots carry `NodeHints` for those chords in place of the tooltip's mouse instruction lines; needs `NodeHints.Chord` installed (a `ChordNames` port). Space picks up any occupied slot, Enter drops through `ArtifactDropUtility`, `DropAccepts` asks `CanRearrangeArtifact`. Escape is the game's (`UI.ExitMenu` registered by `AdventureMenuBackground`) except while carrying. Fixture: the `test` save (Cecilia's Crossbow is two-handed; commander 965 has a two-handed spear in the backpack, an any-hand Falchion and a Kite Shield equipped). | D |
