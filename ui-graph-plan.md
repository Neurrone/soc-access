...
|---|---|---|---|
| `WorldChoiceMenuScreen` | Menu, Buttons, Text, draggable TroopHudMenu | PROPOSED 2026-09-07, awaiting approval: three stops. Troop HUD stop as the market's (the menu rebuilds its cards when troops change, which is why it draws them). Choice (screen name = the drawn title): the body paragraph, then one radio node PER ROW for each card (walked Up/Down), never choosing on arrival (Enter = the game's click = select; a disabled card reads the game's own red reason from its text), then the Confirm button (`unavailable` until a card is selected, live). Close (the band's cross = `ForceClose`, the game's cancel; the map object stays unvisited). Escape is the game's (`AdventureMenuBackground` draws its close cross, so `_canClose` is true) except while carrying. Fixture: `IWorldChoiceMenu.ShowMenu(commander, header, body, List<string>)` resolved from the `WorldChoiceMenuInstaller` container opens the generic variant from the REPL; the reward variant needs `RuntimeRewardDataContainer`s (a `Resource` reward is the safe one). | D |
| `SpellbookScreen` | Menu, DraggableMenu, Checkbox, Buttons | APPROVED 2026-09-07: four stops. The tutorial button (its own stop while drawn). Quickbar: one named region (the drawn header) holding the auto-fill checkbox FIRST (owner's order; the game draws it beneath), then eight slot rows (spell name or empty, position replacing "Slot N"); a filled slot is an expandable group whose one child is the game's hover-revealed delete button as a Remove node; after the slots a synthetic drop target for the game's drag-out-to-nothing removal, labelled as a complete instruction, reading as a plain line while nothing is carried and taking only a spell carried from a slot. Spells: one stop, one region per drawn column named by the game's `SelectedEssenceTitle` (Order spells ... Multi-essence spells); a school region opens with two nodes that own tooltips, the essence income (`(+11)`, essence details) and the tier title (based-on-skill), then its four spells; the multi region is its twenty spells in drawn order. A spell reads name, tier, cost, `unavailable` when the game greys it (`CanCast` false), the detail panel buffer-only. Gestures are the game's: Enter = left click (casts in battle, inert on the map), Backslash = right click = add to the first free slot (the game's own guards: auto-fill off, not already on the bar) with the spoken hint "Backslash adds to quickbar" gated the same way. Carry: Space on a spell entry or a filled slot (null while auto-fill is on), Enter on any slot drops through the entry's own `OnBeginDrag` then `SpellbookMovableSpell` hover + `EndDrag` (the mouse's exact path: move onto empty, swap onto occupied, overwrite from the list; no hover = the game's drag-out removal); sounds are the game's begin, equip and cancel. Close: its own stop. Escape is the game's (`UI.ExitMenu` registered by `SpellBook.Show`) except while carrying; V also closes. | D |
| `TooltipActionsMenuScreen` | Menu | stays a widget screen until no unported screen hands out `TooltipAction`s; deleted in G | G |
| `CommanderSheetScreen` | InventoryGrid, Menu, Buttons, Text | APPROVED 2026-09-07 (ES2's gesture family and usage hints adopted, no mod-authored drop targets): six stops in drawn order: the tutorial button (its own stop while drawn), overview (Stats, Specialization, the three modifier tabs as `Tab` nodes in one row, switching on Enter with the game's own selection as the focus visual as the build menu's tier tabs do, then the active tab's rows under the drawn title), Equipment (one node per slot in drawn order, a two-hander's Main and Off Hand merged into one `InventorySlots/BothHands` node standing for the main hand), Inventory (one node per backpack position, the graph position replacing "slot N"), Skills then Powers (filled entries only), Close. Gestures are the game's own clicks, two actions only: `ui_left_click` (renamed from `ui_activate`; Enter, NumpadEnter, and Ctrl+Enter, Ctrl+NumpadEnter as further bindings) and `ui_right_click` (Backslash, Ctrl+Backslash added). Each binding delivers the same native click and the game's handler reads the physical Ctrl: Ctrl+Enter drops on the ground, Backslash equips/unequips/uses, Ctrl+Backslash destroys (sells in a shop). Auto arrange is a button at the top of the Inventory stop, before the slots. Occupied slots carry three `NodeHints` (right click with the game's own Equip/Unequip/Use word, Ctrl+right click, Ctrl+left click) in place of the tooltip's mouse instruction lines, all spoken on landing as ES2 does; needs `NodeHints.Chord` installed (a `ChordNames` port). Space picks up any occupied slot, Enter drops through `ArtifactDropUtility`, `DropAccepts` asks `CanRearrangeArtifact`. Escape is the game's (`UI.ExitMenu` registered by `AdventureMenuBackground`) except while carrying. Fixture: the `test` save (Cecilia's Crossbow is two-handed; commander 965 has a two-handed spear in the backpack, an any-hand Falchion and a Kite Shield equipped). | D |
| `ArtifactMarketScreen` | InventoryGrid, Menu, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: five stops. Troop HUD: a `Wielder` stop holding the portrait row (wielder name, stats tooltip buffer-only) and a named `Troops` region of one row per drawn slot in drawn order; the region's rows come from ONE shared contributor (`TroopHudRows`) that the map screen declares on its own without the portrait (`Footmen, 19 / 20` or empty, the graph position replacing "slot N", the troop details buffer-only). Gestures are the game's: Enter = left click = nothing (the game drags on button-down), Backslash = right click = disband through the game's confirm (gated on `CanDisband`, hint "Backslash disbands"), Space picks a troop up, Enter on a slot drops through the native `TroopHUDEntryMovable` replay (empty or same troop = the split popup `MoveTroopPopupScreen`, other = swap), Ctrl+Enter as the drop = the game's Ctrl+drop (whole stack, or merge all, or force-split half on itself; the movable reads the physical Ctrl), hint while carrying "Ctrl+Enter moves the whole troop". The game's Ctrl+digit quick-splits (`Adventure.SplitTroopSize1..10`, acting on the hovered slot) are modelled on a focused troop row: the mod claims the chord only there and drives the game's own handler with the row's slot as the hovered entry; the map bookmark chords are scoped to the focused map grid, so nothing collides. Market (screen name = the drawn title): description paragraph, the nine category toggles as one radio row (game tooltips as labels), the filled offers in drawn order (`name (colour), price`, tooltip buffer-only; Enter = the game's click = select for purchase), then the selection band as live nodes (its text line, or the item name and the Buy/Sell button with the price, `unavailable` when the game disables it; refusal is the game's silence). Equipment and Inventory as the commander sheet's two stops, with the market's click meanings: Enter = left click = select for sale, Backslash = equip/unequip, Ctrl+Backslash = sell (the game's sold/cannot-sell notifications), Ctrl+Enter = drop; the same three hints. Close. Escape is the game's (`AdventureMenuBackground`) except while carrying. Fixture: no market exists on the `test` map; spawn `AdventureMapEntities.RaidersMarket` (174) beside the wielder with `CreateAdventureMapEntityCommand` through `IGame.server.Commands` (resolved from the AdventureScene `SceneContext`), open it with `IAdventureMenuSystem.ShowArtifactMarketMenu`; its stock is empty until a round passes, so sell an artifact first for a buy-back offer. | D |
| `TradingScreen` | InventoryGrid, ArmyExchangeGrid, Menu, Buttons | PROPOSED 2026-09-07, awaiting approval (stop order approved): seven stops in a DELIBERATE order, not the drawn one, so a transfer never crosses a wielder stop: left Wielder, right Wielder, left Equipment, left Inventory, right Equipment, right Inventory, Close. A Wielder stop is the sheet's shape (portrait row, Stats region, Modifiers region with the tab row switching on Enter, Troops region, the side's Move all button); the sides are the game's drawn order (chosen by map position). An artifact carried across sides drops through the game's give path (`ArtifactDropUtility` already branches on owner); Backslash on an artifact is the game's right click, which in a trade moves it to the other backpack; Ctrl+Backslash destroys, Ctrl+Enter drops on the ground. Fixture: `IHUDActionSignals.OnTradeInitiated(new TradeInitiatedPayload(a, b))` from the project context. | D |
| `SettlementScreen` | ArmyExchangeGrid, Menu, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: four stops: Tutorial, visiting Wielder (portrait row, Troops region), Settlement (screen name = the drawn building name and custom name; Draft and Upgrade buttons with their drawn description read after the label and `unavailable` as the game disables them; Defending wielder region: status line, Store, Eject, Trade buttons; Settlement troops region: the garrison army rows as carry sources and targets with the two Move all buttons; Garrison and Ballista regions as read-only rows), Close. Escape is the game's (`AdventureMenuBackground`) except while carrying. Fixture: `IAdventureMenuSystem.PurchaseTroopToWielder(townPool, commanderId, Commander)`. | D |
| `DefenceMenuScreen` | ArmyExchangeGrid, Menu, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: Tutorial, the stored Wielder when one is stored (portrait row, Troops region), Defence (screen name = title and subtitle; Draft and Upgrade buttons; the defending wielder's status and Eject button; Settlement troops region with the garrison rows and Move all; Towers region: one row per tower with its tooltip, then the level summary line or the no-towers help; Garrison and Ballista regions), Close. Escape is the game's (`UI.ExitMenu` registered by `DefenceMenu.ShowTopLevel`) except while carrying. Fixture: `IAdventureMenuSystem.OpenDefenceMenu(townEntity)`. | D |
| `HostileJoinMenuScreen` | ArmyExchangeGrid, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: no close cross exists. Choice stage: attacking Wielder (portrait row, Troops region), then Offer (screen name = the title; the body; the joining army as rows identical to a normal army row plus `unavailable` while the offer is locked; No, then Yes with the gold price, `unavailable` when unaffordable). Join stage: Wielder, then Joining (the instruction text, the joining rows as carry sources, Move all, Discard which the game relabels Close once the army is empty). Escape is the game's: No in the choice stage, Done in the join stage. | D |
...
| `MoveTroopPopupScreen` | Slider, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: a child dialog over the screen whose troop rows opened it (screen name = the drawn header, `Move troops`), one stop: the drawn `Max Troop Size 20` line as text, the slider (`Slider` node named by the game's header, value read as the two drawn amounts `Left: 18, right: 1`, Left/Right adjust through the native `HandleSliderChanged`, Enter = the game's slider release `HandleSliderPointerUp` which commits the move), then the three drawn buttons in one row: move all left, split equal, move all right (the game's tooltips as their labels; `unavailable` as the game disables them; the split button also answers the game's own `FocusSelected` key). No OK or Cancel exists in the game: Enter on the slider commits, Escape is the game's `UI.Cancel` (the popup polls it), a mod-claimed Back only while carrying is impossible here since the carry ended at the drop. The Hotkeys hint icon's tooltip (quick split, split half) is buffer-only on the slider. Opened by a carry drop on an empty slot or onto the same troop; a drop with Ctrl held skips it. Fixed 2026-09-07 (`TroopHudAdapter.WakeMovable`): the replay used to call `BeginDrag` and `DecideAmount` in the frame that first activated a wielder band's drag ghost, before Unity ran the ghost's `Start()` (which assigns `_mainRectTransform`), so `DecideAmount` threw and switched the mod's update handler off. Follow-up: `MoveTroopPopupAdapter.IsPresent` reads an idle ghost that is merely active (state `Empty`, dialog container active) as a popup, so a resync can push a bogus `MoveTroopPopupScreen`; gate it on the state. | D |
