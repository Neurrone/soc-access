...
| `CommanderSheetScreen` | InventoryGrid, Menu, Buttons, Text | PROPOSED 2026-09-07, awaiting approval: five stops in drawn order: overview (tutorial button, Stats, Specialization, one modifier node stepping the game's `SwitchTab` axis with the active tab's rows under the drawn title), Equipment (one node per slot in drawn order, a two-hander's Main and Off Hand merged into one `InventorySlots/BothHands` node standing for the main hand), Inventory (one node per backpack position, the graph position replacing "slot N", then two mod-authored drop targets Ground and Destroy and an Auto arrange button), Powers + Skills (filled entries only), Close. Space picks up any occupied slot, Enter drops through `ArtifactDropUtility`, `DropAccepts` asks `CanRearrangeArtifact`, Enter unheld is the game's right-click equip toggle. Escape is the game's (`UI.ExitMenu` registered by `AdventureMenuBackground`) except while carrying. Fixture: the `test` save (Cecilia's Crossbow is two-handed; commander 965 has a two-handed spear in the backpack, an any-hand Falchion and a Kite Shield equipped). | D |
