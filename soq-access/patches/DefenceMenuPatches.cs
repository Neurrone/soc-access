...
namespace SongsOfConquestAccess
{
    ...
    internal static class DefenceMenuPatches
    {
        ...
        private static void DefenceMenuHidePostfix(DefenceMenu __instance, bool __state)
        {
            if (__state)
            {
                SoqAccessPlugin.Instance?.ScreenDetector?.OnDefenceMenuClosed(__instance);
            }
        }
        ...
        private static IEnumerator WaitForTopLevel(DefenceMenu menu, bool fromSubMenu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                if (adapter.IsTopLevelPresent())
                {
                    if (fromSubMenu)
                    {
                        SoqAccessPlugin.Instance?.ScreenDetector?.OnDefenceMenuBackToTop(menu);
                    }
                    else
                    {
                        SoqAccessPlugin.Instance?.ScreenDetector?.OnDefenceMenuReady(menu);
                    }
        ...
        private static IEnumerator WaitForDraft(DefenceMenu menu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                {
                    SoqAccessPlugin.Instance?.ScreenDetector?.OnDefenceDraftReady(menu);
                    yield break;
                }
        ...
        private static IEnumerator WaitForUpgrade(DefenceMenu menu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                {
                    SoqAccessPlugin.Instance?.ScreenDetector?.OnDefenceUpgradeReady(menu);
                    yield break;
                }
