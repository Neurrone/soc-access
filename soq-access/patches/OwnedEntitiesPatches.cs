...
{
    [HarmonyPatch]
    internal static class OwnedEntitiesPatches
    {
        ...
        {
            SoqAccessPlugin.Instance?.ScreenDetector?.OnOwnedEntitiesReady(__instance);
        }
        ...
        {
            SoqAccessPlugin.Instance?.ScreenDetector?.OnOwnedEntitiesClosed(__instance);
        }
        ...
        private static IMapEntity GetCategoryParent(KingdomEntityOverviewCategoryEntry entry)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("OwnedEntitiesPatches failed to read category parent: " + ex.Message);
                return null;
            }
        }
        ...
        private static IMapEntity GetCurrentEntity(KingdomEntityOverviewClaimedEntry entry)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("OwnedEntitiesPatches failed to read current owned entity row target: " + ex.Message);
                return null;
            }
        }
    }
