...
namespace SongsOfConquestAccess.Adapters
{
    ...
    internal sealed class CombatAdapter
    {
        ...
        private List<BattleAttackPreview> GetActiveAttackPreviews()
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to capture attack preview text: " + exception.Message);
            }
        ...
        private ITooltipable GetBattleTooltipable()
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to resolve battle tooltip behavior: " + exception.Message);
                return null;
            }
        }
        ...
        public void SetFocusedTileOverlay(Vector2Int tile)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to set focused tile overlay: " + exception.Message);
            }
        }
        ...
        public void ClearFocusedTileOverlay()
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to clear focused tile overlay: " + exception.Message);
            }
        }
        ...
        private bool TryBeginScreenInputOverride(Vector2Int tilePosition, out ScreenInputOverride screenInputOverride)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter could not override native screen input because primary screen input was unavailable");
                return false;
            }
            ...
            if (response == null)
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter could not override native screen input because no writable ScreenInputResponse could be resolved from " + _inputManager.Screen.Primary.GetType().FullName);
                return false;
            }
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter could not target tile " + FormatPoint(tilePosition) + " because its screen position is outside the current view: " + screenPosition);
                return false;
            }
        ...
        private void UpdateNativeAttackPreviews()
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to update native attack previews: " + exception.Message);
            }
        }
        ...
        private Vector3 GetWorldCenter(Vector2Int tile)
        {
            if (_pointToWorldMethod != null)
            {
                ...
                {
                    SoqAccessPlugin.Instance?.LogWarning("CombatAdapter failed to resolve tile world position: " + exception.Message);
                }
            }
        ...
        private sealed class ScreenInputOverride
        {
            ...
            public static ScreenInputOverride Apply(object response, Vector2 screenPosition)
            {
                ...
                {
                    SoqAccessPlugin.Instance?.LogWarning("CombatAdapter could not override native screen input because required writable properties were missing on " + responseType.FullName);
                    return null;
                }
