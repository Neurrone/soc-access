...
namespace SongsOfConquestAccess.Adapters
{
    internal sealed class BattleHudAdapter
    {
        ...
        private static readonly FieldInfo AbilityInstructionNameField =
            AccessTools.Field(typeof(AbilityTargetInstruction), "_abilityName");
        private static readonly FieldInfo AbilityInstructionTextField =
            AccessTools.Field(typeof(AbilityTargetInstruction), "_targetInstruction");
        private static readonly FieldInfo AbilityInstructionCanvasGroupField =
            AccessTools.Field(typeof(AbilityTargetInstruction), "_canvasGroup");
        private static readonly FieldInfo TroopStatusPanelTroopStateField =
            AccessTools.Field(typeof(BattleTroopStatusPanel), "_troopState");
        ...
        public string TargetingInstructionText
        {
            get
            {
                string abilityInstruction = GetVisibleAbilityInstructionText();
                if (!string.IsNullOrWhiteSpace(abilityInstruction))
                {
        ...
        {
            GameLogHandleUI handle = FindLiveComponent<GameLogHandleUI>();
            if (handle != null)
            {
        ...
        private BattleTroopStatusPanel GetCurrentTroopStatusPanel()
        {
            ...
            BattleTroopStatusPanel[] panels = Resources.FindObjectsOfTypeAll<BattleTroopStatusPanel>();
            for (int i = 0; i < panels.Length; i++)
            {
                BattleTroopStatusPanel panel = panels[i];
                if (!IsGameObjectVisible(panel))
                {
                    continue;
                }

                IBattleTroopState troop = GetField<IBattleTroopState>(panel, TroopStatusPanelTroopStateField);
                if (troop != null && troop.Id == current.Id)
                {
                    return panel;
                }
            }
        ...
        private string GetVisibleAbilityInstructionText()
        {
            AbilityTargetInstruction[] instructions = Resources.FindObjectsOfTypeAll<AbilityTargetInstruction>();
            for (int i = 0; i < instructions.Length; i++)
            {
                AbilityTargetInstruction instruction = instructions[i];
                if (!IsGameObjectVisible(instruction))
                {
                    continue;
                }

                CanvasGroup canvasGroup = GetField<CanvasGroup>(instruction, AbilityInstructionCanvasGroupField);
                if (canvasGroup != null && canvasGroup.alpha <= 0.01f)
                {
                    continue;
                }

                string name = GetText(GetField<UITextMesh>(instruction, AbilityInstructionNameField));
                string text = GetText(GetField<UITextMesh>(instruction, AbilityInstructionTextField));
        ...
        private T FindLiveComponent<T>() where T : Component
        {
        ...
        internal sealed class QuickbarItem
        {
            ...
            {
                get { return Tooltip.ForComponent(_adapter.GetQuickbarEntryButton(_entry), _adapter._localization); }
            }
        }
        ...
        internal sealed class QueueItem
        {
            ...
            public string Label
            {
                get
                {
                    ...
                    return Index + ", " + _adapter.GetTroopLabel(TroopId, includePosition: false);
                }
            }
