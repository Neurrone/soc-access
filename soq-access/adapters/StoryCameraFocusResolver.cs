...
namespace SongsOfConquestAccess.Adapters
{
    internal static class StoryCameraFocusResolver
    {
            ...
            CameraFocusPointIdentifier camera)
        {
            ...
            if (focusPoint == null)
            {
                SoqAccessPlugin.Instance?.LogWarning("Story camera focus point not found: " + camera.reference);
                return null;
            }
        ...
        public static string LocalizeName(ILocalizationHandler localizationHandler, string nameKey, int pluralCount)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("Failed to localize story camera focus target " + nameKey + ": " + exception.Message);
                return nameKey;
            }
        }
        ...
        private static IMapEntity FindMeaningfulEntityAt(IClientAdventureFacade facade, Vector2Int tile, IMapEntity focusPoint)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("Failed to resolve story camera focus tile entity at " + FormatTile(tile) + ": " + exception.Message);
            }
        ...
        private static Vector2Int? WorldToTile(object cartographyConverter, Vector3 worldPosition)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("Failed to convert story camera focus world position to tile: " + exception.Message);
            }
        ...
        private static string SafeCommanderName(IClientAdventureFacade facade, int commanderId)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("Failed to resolve story camera focus wielder name: " + exception.Message);
                return "wielder";
            }
        }
