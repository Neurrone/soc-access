    ...
    internal sealed class PauseMenuAdapter
    {
        ...
        private PauseMenu.Settings Settings
        {
            get
            {
                ...
                {
                    SoqAccessPlugin.Instance?.LogWarning("Failed to read PauseMenu settings: " + exception.Message);
                    return null;
                }
            }
        }
        ...
        private static void AddItem(List<Item> items, string id, UIButton button)
        {
            ...
            {
                SoqAccessPlugin.Instance?.LogWarning("Skipping visible pause menu button with empty direct text: " + id);
                return;
            }
        ...
        private sealed class NativeScreenInputPositionOverride : System.IDisposable
        {
            ...
            public static NativeScreenInputPositionOverride Apply(Vector2 position)
            {
                ...
                if (response == null)
                {
                    SoqAccessPlugin.Instance?.LogWarning("PauseMenuAdapter could not override native screen input position");
                    return null;
                }
                ...
                {
                    SoqAccessPlugin.Instance?.LogWarning("PauseMenuAdapter could not override native screen input position because Position was not writable on " + response.GetType().FullName);
                    return null;
                }
