...
namespace SongsOfConquestAccess.Adapters
{
    ...
    public sealed class TradingMenuAdapter : IPresent
    {
        ...
        public sealed class Side : IArtifactSlots
        {
            ...
            // This side's artifacts, read the way every menu that draws an InventoryHUD reads
            // them. The reader keeps the two lists while the game's own inventory is unchanged,
            // which cost 0.62 ms of a 1.42 ms build to rebuild.
            ...
            public Side(TradingMenuAdapter owner, bool left)
            {
                ...
                _slots = new InventorySlotReader(
                    ...
                    owner._facade,
                    owner._localization,
                    owner._artifactLookup,
                    "TradingMenuAdapter",
                    InventorySlotReader.TradeMouseInstructionKeys);
            }
            ...
            public IReadOnlyList<LabeledItem> GetStats()
            {
                ...
                AddStat(items, StatEntryType.Offense, GameText.Get(_owner._localization, "Commanders/Tooltip/Offense", string.Empty), commander.Stats.Offense.GetValue());
                AddStat(items, StatEntryType.Defense, GameText.Get(_owner._localization, "Commanders/Tooltip/Defense", string.Empty), commander.Stats.Defense.GetValue());
                AddStat(items, StatEntryType.Movement, GameText.Get(_owner._localization, "Commanders/Tooltip/Movement", string.Empty), (int)commander.Stats.Movement.GetValue());
                AddStat(items, StatEntryType.View, GameText.Get(_owner._localization, "Commanders/Tooltip/ViewRadius", string.Empty), (int)commander.Stats.ViewRadius.GetValue());
                return items;
            }
            ...
            /// <summary>This wielder's equipment and backpack, and every gesture the game gives an
            /// artifact in them, read by the shared inventory reader. The right click is this menu's
            /// own: it MOVES THE ARTIFACT TO THE OTHER BACKPACK
            /// (<c>InventoryHUD.EquipArtifact</c> answers a set other inventory with
            /// <c>MoveItemToOtherBackpack</c>), and a drop on the other side goes down the game's
