...
{
    [HarmonyPatch]
    internal static class TownInteractionMenuPatches
    {
        ...
        private static void TownInteractionMenuClosePostfix(TownInteractionMenu __instance, bool __state)
        {
            if (__state)
            {
                SocAccessPlugin.Instance?.ScreenDetector?.OnSettlementClosed(__instance);
            }
        }
        ...
        private static IEnumerator WaitForTopLevel(TownInteractionMenu menu, bool fromSubMenu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                if (adapter.IsTopLevelPresent())
                {
                    if (fromSubMenu)
                    {
                        SocAccessPlugin.Instance?.ScreenDetector?.OnSettlementBackToTop(menu);
                    }
                    else
                    {
                        SocAccessPlugin.Instance?.ScreenDetector?.OnSettlementReady(menu);
                    }
        ...
        private static IEnumerator WaitForDraft(TownInteractionMenu menu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                {
                    SocAccessPlugin.Instance?.ScreenDetector?.OnSettlementDraftReady(menu);
                    yield break;
                }
        ...
        private static IEnumerator WaitForUpgrade(TownInteractionMenu menu)
        {
            ...
            while (menu != null && frames < 120)
            {
                ...
                {
                    SocAccessPlugin.Instance?.ScreenDetector?.OnSettlementUpgradeReady(menu);
                    yield break;
                }
