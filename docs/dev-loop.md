...
- The REPL cannot name an `AutoGeneratedDef` enum: pass `Enum.ToObject(parameterType, n)`
  where a game method takes one. Reusing a top-level variable name with another type in the
  same load fails with "Internal compiler error"; pick a new name.
- A game menu the mod tracks through a patch's static bookkeeping (`CombatPatches.ActivePostBattleMenus`)
